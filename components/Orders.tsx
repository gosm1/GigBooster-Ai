...
// Generate unique notifications with IDs
notifications = Array.from({ length: 10 }, (_, i) => 
  notifications.map(n => ({ ...n, id: `${n.id}-${i}` }))
).flat();
  ...
  return (
    <section id="benefits">
      <div className="bg-black flex flex-col sm:flex-row items-center justify-evenly md:pt-16">
        <h1 className="text-2xl text-center px-4 md:px-0 md:text-start pb-10 md:pb-0 md:text-4xl max-w-lg font-normal">
          This Is How{" "}
          <span className="font-semibold bg-gradient-to-r from-[#F1EFFE] via-[#B3A7FE] via-[#804eec] to-[#4325FF] bg-clip-text text-transparent">
            Your Orders
          </span>{" "}
          Will Look After Using GigBooster AI
        </h1>
        <div
          className={cn(
